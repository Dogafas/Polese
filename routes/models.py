...
class Route(models.Model):
    ...
    departure_location = models.CharField(max_length=255, verbose_name="Пункт отправления")
...
class RoutePoint(models.Model):
    route_point_id = models.AutoField(primary_key=True, verbose_name="ID точки маршрута")
    route = models.ForeignKey(Route, on_delete=models.CASCADE, verbose_name="Маршрут")
    stop = models.ForeignKey(Stop, on_delete=models.CASCADE, verbose_name="Остановка")
    order = models.IntegerField(verbose_name="Порядок", validators=[MinValueValidator(0)])
    arrival_time = models.TimeField(verbose_name="Время прибытия")
    departure_time = models.TimeField(verbose_name="Время отправления", null=True, blank=True)
    price_multiplier = models.DecimalField(max_digits=5, decimal_places=2, verbose_name="Множитель цены", null=True, blank=True )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Дата создания")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="Дата обновления")
