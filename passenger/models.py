...
def validate_passport_series(value):
    ...
    if value and not re.fullmatch(r'\d{4}', value): # <-ЗДЕСЬ ИЗМЕНЕНИЯ: (Проверка на пустое значение)
        raise ValidationError("Некорректный формат серии паспорта. Ожидаемый формат: 'XXXX'")


def validate_passport_number(value):
    """Проверка корректности номера паспорта."""
    if value and not re.fullmatch(r'\d{6}', value): # <-ЗДЕСЬ ИЗМЕНЕНИЯ: (Проверка на пустое значение)
        raise ValidationError("Некорректный формат номера паспорта. Ожидаемый формат: 'XXXXXX'")
...
class Passenger(models.Model):
    ...
    phone_number = PhoneNumberField(blank=False, region='RU', verbose_name="Номер телефона")
